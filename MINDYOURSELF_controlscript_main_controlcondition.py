...
df_fake_eeg = np.array(pd.read_excel("fake_participant.xlsx"))
...
def main():
    ...
    try:
        while True:
            ...
            if timestamps_arval:
                real_ar = max_min_int(chunk_arval[0][0])  # arousal extracted via OpenVibe
                real_valence = max_min_int(chunk_arval[0][1])  # valence extracted via OpenVibe
                print("real arousal: " + str(real_ar) + ", real valence: " + str(real_valence))

                fake_ar = int(df_fake_eeg[i][0])
                fake_valence = int(df_fake_eeg[i][1])

                old_list_of_indices = np.array(list_of_indices)
                list_of_changes = update_notes(list_of_indices, fake_ar, fake_valence)
            ...
            if timestamps_perfmet:
                ...
                real_perfmet = np.array(chunk_perfmet[0][1:])
                print("real perfmet: " + str(real_perfmet))


                now = datetime.now()
                data[now] = [real_ar, real_valence, real_perfmet]

                fru, eng, val, exc, foc, med = df_fake_eeg[i][2:]
        ...
        df.to_csv(('placebo_participant' + str(participant_number) +'.csv'))
